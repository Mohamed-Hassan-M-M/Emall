                ...
                <div class="col-12">
                    <div class="text-right mb-3">
                        ...
                        <button type="button" id="submitBtn"
                                class="btn w-sm btn-success waves-effect waves-light">{{__('manager.create')}}</button>
                    </div>
                </div> <!-- end col-->
            </div>
        </div>
...
<script>
    ...
    class Item {
        ...
        createViewFromData(features, price, revenue, quantity) {
            this.price = price;
    ...
    class ProductItem {
        ...
        addItemWithData(element, index) {
            ...
            const horizontalRule = document.createElement('hr');
            const item = new Item();
            const itemRow = item.createViewFromData(element.product_item_features, element.price, element.revenue, element.quantity);
            this.productItemWrapper.appendChild(itemHeaderRow);
            this.productItemWrapper.appendChild(itemRow);
            this.productItemWrapper.appendChild(horizontalRule);
            this.items.push(item);
    ...
    $(document).ready(function () {
        ...
        const oldItems = '{{old('items')}}'.replace(/&quot;/g, '"');
        const productItem = new ProductItem();
        if (oldItems != "") {
            productItem.createViewWithData(JSON.parse(oldItems))
        } else {
