<div class="form-group mt-0">
    ...
    <input type="text" class="form-control @if($errors->has('title_'.$item->code)) is-invalid @endif" id="title_{{$item->code}}" placeholder="{{__('admin.category_'.$item->code)}}" name="title_{{$item->code}}" value="{{ old('title_'.$item->code, (isset($subCategory))?$subCategory->getTranslation('title', $item->code):'')}}">
...
<div class="form-group">
    ...
    <textarea name="description_{{$item->code}}" id="description_{{$item->code}}" class="form-control @if($errors->has('description_'.$item->code)) is-invalid @endif"
              placeholder="{{__('admin.description_'.$item->code)}}">{{ old('description_'.$item->code, (isset($subCategory))?$subCategory->getTranslation('description', $item->code):'')}}</textarea>
