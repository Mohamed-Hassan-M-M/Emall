...
class OrderController extends Controller
{
    public function index()
    {
        ...
        if ($shop) {
            $orders =  Order::with('carts','carts.product','carts.product.productImages','orderPayment')
            ...
            if ($order) {
                    ...
                    if (Order::isCancellable($order->status)) {
                        ...
                        if ($order->save()) {
                            ...
                            return redirect()->back()->with([
                                'message' => trans('manager.order_cancelled')
                            ]);
                        } else {
                            return redirect()->back()->with([
                        ...
                        return redirect()->back()->with([
                            'error' => trans('manager.cant_cancel_order')
                        ]);
                    }
                }
                ...
                if(Order::isOrderTypePickup($order->order_type)){
                    if ($request->get('status') > 4) {
                        return redirect()->back()->with([
                            'error' => trans('manager.you_cant_perform')
                        ]);
                    }
                }else {
                ...
                if ($request->get('status') == 2) {
                    foreach ($order->carts as $cart) {
                        if ($cart->productItem->quantity < $cart->quantity) {
                            return redirect()->back()->with([
                                'error' => trans('manager.not_enough_quantity')
                            ]);
                        }
                    }
                    foreach ($order->carts as $cart) {
                ...
                if ($order->save()) {
                    return redirect()->back()->with([
                        'message' => trans('manager.order_changed')
                    ]);
                } else {
                    return redirect()->back()->with([
                        'error' => trans('manager.something_wrong')
