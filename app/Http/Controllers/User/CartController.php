...
class CartController extends Controller
{
        ...
        if (!$cart) {
            if($quantity > $productItem->quantity){
                return redirect()->back()->with([
                    'error' => trans('user.quantity_not_enough')
                ]);
            }

            if(!$product->active){
                return redirect()->back()->with([
                    'error' => trans('user.product_not_active')
                ]);
            }
            ...
            if ($cart->save()) {
                return redirect()->back()->with([
                    'message' => trans('user.product_added_to_cart')
                ]);
            }
            ...
            return redirect()->back()->with([
                'error' => trans('user.product_already_in_cart')
            ]);
        }
    ...
    public function destroy(Request $request)
    {
        ...
        if($cart->delete()){
            return redirect()->back()->with([
                'message' => trans('user.cart_deleted')
            ]);
        }else{
            return redirect()->back()->with([
                'error' => trans('user.something_wrong')
