    ...
    public function store(Request $request) {
        ...
        if (count($carts) == 0) {
            return redirect(route('user.orders.index'))->with([
                        'error' => trans('user.order_already_placed')
            ]);
        }
        ...
        foreach ($carts as $cart) {
            if ($cart->order_id) {
                return redirect()->back()->with([
                            'error' => trans('user.cart_another_order')
                ]);
            }
            if ($singleShopId != $cart->shop_id) {
                return redirect()->back()->with([
                            'error' => trans('user.order_same_shop')
                ]);
            }
        }
        ...
        if ($orderPayment) {
            ...
            foreach ($carts as $cart) {
                ...
                $cart->p_name = $product->getTranslations('name');
                $cart->p_description = $product->getTranslations('description');
    ...
    public function update(Request $request, $id) {
            ...
            if (Order::isCancelStatus($request->status)) {
                if (Order::isCancellable($order->status)) {
                    ...
                    if ($order->save()) {
                        ...
                        return redirect(route('user.orders.index'))->with([
                                    'message' => trans('user.order_cancelled')
                        ]);
                    } else {
                        return redirect()->back()->with([
                                    'error' => trans('user.order_status_not_changed')
                        ]);
                    }
                } else {
                    return redirect()->back()->with([
                                'error' => trans('user.order_already_accepted')
                    ]);
                }
            }
        }
        ...
        if (isset($request->success) & isset($request->payment_id)) {
            ...
            if ($orderPayment->save() && $order->save()) {
                ...
                return redirect(route('user.orders.index'))->with([
                            'message' => trans('user.payment_method_updated')
                ]);
            } else {
                return redirect()->back()->with([
                            'error' => trans('user.payment_failed')
                ]);
            }
            ...
            if ($request->status == 5) {
                ...
                if (!ShopRevenueController::storeRevenue($id)) {
                    return redirect()->back()->with([
                                'error' => trans('user.delivery_is_wrong')
                    ]);
                }
                $order->status = $request->status;
                if ($order->save()) {
                    ...
                    return redirect()->back()->with([
                                'message' => trans('user.order_delivered')
                    ]);
                }else {
                    return redirect()->back()->with([
                                'error' => trans('user.order_status_not_changed')
